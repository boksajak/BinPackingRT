...
void Audio::Initialize(HWND hwnd)
{
	...
	utils::validate(hr, L"Failed to create XAudio2 device");

	hr = mpXAudio2->CreateMasteringVoice(&mpMasterVoice);
	utils::validate(hr, L"Failed to create XAudio2 mastering voice");
}
...
void Audio::Cleanup()
{
	...
	if (mpMasterVoice) mpMasterVoice->DestroyVoice();

	SAFE_RELEASE(mpXAudio2);
}

SoundHandle Audio::LoadSound(FileSystem& fileSystem, std::wstring fileName)
{
	FileHandle soundFile = fileSystem.openFile(fileName);

	size_t soundFileSize;
	unsigned char* soundFileData = fileSystem.readAll(soundFile, soundFileSize);

	SoundData soundData;
	HRESULT hr = LoadWAVAudioInMemoryEx(soundFileData, soundFileSize, soundData.waveData);
	utils::validate(hr, L"Could not load wave sound file!");
	if (FAILED(hr)) return SIZE_MAX;

	// Create the source voice
	if (FAILED(hr = mpXAudio2->CreateSourceVoice(&soundData.pSourceVoice, soundData.waveData.wfx)))
	{
	...
	soundData.buffer.Flags = XAUDIO2_END_OF_STREAM;  // tell the source voice not to expect any data after this buffer
	soundData.buffer.AudioBytes = soundData.waveData.audioBytes;

	if (soundData.waveData.loopLength > 0)
	{
		soundData.buffer.LoopBegin = soundData.waveData.loopStart;
		soundData.buffer.LoopLength = soundData.waveData.loopLength;
		soundData.buffer.LoopCount = 0;
	}
...
void Audio::SoundPlay(SoundHandle soundHandle)
{
	if (soundHandle == SIZE_MAX) return;

	SoundData* soundData = &sounds[soundHandle];
	...
	for (unsigned int i = 0; i < repeats; i++) {
		if (soundData->waveData.seek)
		{
			soundData->xwmaBuffer.pDecodedPacketCumulativeBytes = soundData->waveData.seek;
			soundData->xwmaBuffer.PacketCount = soundData->waveData.seekCount;
			if (FAILED(hr = soundData->pSourceVoice->SubmitSourceBuffer(&soundData->buffer, &soundData->xwmaBuffer)))
			{
		...
		else if (FAILED(hr = soundData->pSourceVoice->SubmitSourceBuffer(&soundData->buffer)))
		{
